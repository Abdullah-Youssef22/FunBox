...
bool SUS_Board::is_win(Player<char>* player) {
    ...
    for (int i = 0; i < rows; ++i) {
        if ((SUS_win(board[i][0], board[i][1], board[i][2])) ||
            (SUS_win(board[0][i], board[1][i], board[2][i])))
            return true;
    }

    // Check diagonals
    if ((SUS_win(board[0][0], board[1][1], board[2][2])) ||
        (SUS_win(board[0][2], board[1][1], board[2][0])))
        return true;
...
SUS_UI::SUS_UI() : UI<char>("Weclome to SUS Game in FUN BOX", 3) {}
